---
description: 로컬 개발 서버를 기동합니다. Supabase 로컬 Postgres 컨테이너를 띄우고(필요 시), .env.local을 최신 값으로 동기화한 뒤, Next.js 개발 서버(npm run dev)를 실행합니다.
---
...
- `drizzle/` 디렉토리가 없으면 "스키마가 아직 없습니다. Gemini에게 tasks 테이블 정의를 요청하세요"라고 안내하고 다음 단계로 넘어감.
